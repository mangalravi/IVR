			...
			return function (row, update, render, closeCallback) {
				if (!update) {
					...
					var modal = $('<div className="dtr-modal"/>')
						.append(
							$('<div className="dtr-modal-display"/>')
								.append(
								...
								.append(
									$('<div className="dtr-modal-close">&times;</div>').click(function () {
										close();
									})
								)
						)
						.append(
							$('<div className="dtr-modal-background"/>').click(function () {
								close();
							})
						)
						.appendTo('body');
	...
	Responsive.renderer = {
		listHiddenNodes: function () {
			return function (api, rowIdx, columns) {
				...
				var data = $.each(columns, function (i, col) {
					if (col.hidden) {
						var klass = col.className ? 'className="' + col.className + '"' : '';
						...
						$(
							...
							.append(
								$('<span className="dtr-data"/>').append(
		...
		tableAll: function (options) {
			...
			return function (api, rowIdx, columns) {
				...
				return $('<table className="' + options.tableClass + ' dtr-details" width="100%"/>').append(
					data
				);
			};
		}
